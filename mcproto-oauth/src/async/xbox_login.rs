//! Shared Xbox Live, XSTS, and Minecraft Services token exchange.
...
/// Exchanges Microsoft access tokens into Minecraft Services access tokens.
///
/// This stage is independent of how the Microsoft token was acquired and is
/// shared by redirect and device-code login flows.
#[derive(Clone)]
...
impl XboxLogin {
    ...
    /// Creates a token exchanger using an application-provided HTTP client.
    pub fn with_http_client(http: Client) -> Self {
        Self { http }
    }

    /// Completes every shared login stage and retains all intermediate results.
    pub async fn login(&self, microsoft: MicrosoftToken) -> Result<LoginResult, Error> {
        let xbox_live = self.authenticate_xbox_live(&microsoft.access_token).await?;
        let xsts = self.authorize_xsts(&xbox_live).await?;
        let minecraft = self.authenticate_minecraft(&xsts).await?;
        Ok(LoginResult {
            microsoft,
            xbox_live,
            xsts,
    ...
    pub async fn authenticate_xbox_live(
        ...
        let response = self
            ...
            .send()
            .await?;
        let token: RawXboxToken = decode_json(response, "Xbox Live").await?;
    ...
    pub async fn authorize_xsts(&self, xbox_live: &XboxLiveToken) -> Result<XstsToken, Error> {
        let response = self
            .http
            .post(XSTS_ENDPOINT)
            .json(&serde_json::json!({
            ...
            .send()
            .await?;
        let token: RawXboxToken = decode_json(response, "XSTS").await?;
        let user_hash = token.user_hash()?;
        Ok(XstsToken {
            issue_instant: token.issue_instant,
            not_after: token.not_after,
    ...
    pub async fn authenticate_minecraft(&self, xsts: &XstsToken) -> Result<MinecraftToken, Error> {
        ...
        let response = self
            ...
            .send()
            .await?;
        decode_json(response, "Minecraft Services").await
    }
    ...
    pub async fn minecraft_token(
        ...
        let xbox_live = self.authenticate_xbox_live(microsoft_access_token).await?;
        let xsts = self.authorize_xsts(&xbox_live).await?;
        self.authenticate_minecraft(&xsts).await
    }
}
