...
//!
//! A session can run the complete Microsoft-to-Minecraft login automatically,
//! or expose each polling and token-exchange stage to the application.
...
/// Configuration and HTTP client for Microsoft device-code login.
#[derive(Clone)]
...
impl DeviceCodeFlow {
    /// Creates a device-code flow for an application-owned Microsoft client ID.
    ...
    /// Requests a device code and creates a polling session.
    pub async fn start(&self) -> Result<DeviceCodeSession, Error> {
        let response = self
            .http
        ...
        let authorization: RawDeviceAuthorization =
            decode_json(response, "Microsoft device authorization").await?;
    ...
    pub async fn exchange_refresh_token(
        ...
        let response = self
            ...
            .send()
            .await?;
        decode_json(response, "Microsoft device token").await
    }

    /// Refreshes the Microsoft token, then completes all shared login stages.
    ...
    async fn poll_token(&self, device_code: &Secret) -> Result<RawDevicePoll, Error> {
        let response = self
            .http
            .post(TOKEN_ENDPOINT)
            .form(&[
            ...
            .send()
            .await?;
        let (status, bytes) = read_response(response, "Microsoft device token").await?;
        parse_poll_response(status, &bytes)
    }
}

/// A device authorization and its polling state.
pub struct DeviceCodeSession {
    ...
    /// Verification URL containing the user code, when Microsoft supplies one.
...
impl DeviceCodeSession {
    ...
    ///
    /// This increases by five seconds whenever Microsoft returns `slow_down`.
    ...
    ///
    /// Callers using this low-level method must respect [`poll_interval`](Self::poll_interval).
    pub async fn poll_once(&mut self) -> Result<DeviceCodePoll, Error> {
        if self.remaining().is_zero() {
            return Err(expired_locally());
        }
        match self.flow.poll_token(&self.device_code).await? {
    ...
    /// Polls at Microsoft's requested interval until a Microsoft token is available.
    pub async fn wait_for_token(mut self) -> Result<MicrosoftToken, Error> {
        loop {
    ...
    /// Waits for authorization, then completes Xbox, XSTS, and Minecraft login.
    pub async fn complete(self) -> Result<LoginResult, Error> {
        let http = self.flow.http.clone();
        let microsoft = self.wait_for_token().await?;
        XboxLogin::with_http_client(http).login(microsoft).await
    }
}
...
pub enum DeviceCodePoll {
    ...
    /// The user authorized the application and Microsoft returned tokens.
