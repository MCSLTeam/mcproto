//! Authorization-code login with PKCE and a local redirect listener.
...
/// Configuration and HTTP client for the redirect login flow.
#[derive(Clone)]
...
impl RedirectFlow {
    /// Creates a redirect flow for an application-owned Microsoft client ID.
    ///
    /// `redirect_uri` must exactly match a URI registered for the client ID.
    /// Only local HTTP callback URLs on `localhost` or a loopback IP are
    /// accepted because this flow starts a local listener.
    ...
    /// Replaces the HTTP client used for Microsoft and Xbox token exchanges.
    ...
    pub async fn start(&self) -> Result<RedirectSession, Error> {
        let host = self
            .redirect_uri
            .host_str()
    ...
    /// Exchanges an authorization code and completes the shared Xbox login stage.
    pub async fn login_with_code(&self, code: AuthorizationCode) -> Result<LoginResult, Error> {
        let microsoft = self.exchange_code(&code).await?;
        XboxLogin::with_http_client(self.http.clone())
            .login(microsoft)
            .await
    }

    /// Exchanges a Microsoft refresh token, then completes the shared Xbox login stage.
    pub async fn login_with_refresh_token(
        &self,
        refresh_token: &Secret,
    ) -> Result<LoginResult, Error> {
        let microsoft = self.exchange_refresh_token(refresh_token).await?;
        XboxLogin::with_http_client(self.http.clone())
            .login(microsoft)
            .await
    }
    ...
    pub async fn exchange_code(&self, code: &AuthorizationCode) -> Result<MicrosoftToken, Error> {
        let response = self
            .http
            .post(TOKEN_ENDPOINT)
            .form(&[
            ...
            .send()
            .await?;
        decode_json(response, "Microsoft OAuth").await
    }
...
/// A bound local listener and its corresponding OAuth authorization request.
...
impl RedirectSession {
    /// Returns the Microsoft URL the user should open in a browser.
    pub fn authorization_url(&self) -> &Url {
        &self.authorization_url
    }

    /// Returns the generated PKCE verifier before the callback is received.
    pub fn code_verifier(&self) -> &Secret {
        &self.code_verifier
    }

    /// Returns the S256 challenge derived from the PKCE verifier.
    pub fn code_challenge(&self) -> &str {
        &self.code_challenge
    }

    /// Returns the generated OAuth state before the callback is received.
    pub fn state(&self) -> &Secret {
        &self.state
    }

    /// Waits for the redirect and returns the verified authorization code.
    pub async fn receive_code(self) -> Result<AuthorizationCode, Error> {
        loop {
    ...
    /// Waits for the redirect and completes Microsoft, Xbox, XSTS, and Minecraft login.
    pub async fn complete(self) -> Result<LoginResult, Error> {
        let flow = self.flow.clone();
        let code = self.receive_code().await?;
        flow.login_with_code(code).await
    }
}
...
async fn read_callback_request(stream: &mut tokio::net::TcpStream) -> Result<Vec<u8>, Error> {
